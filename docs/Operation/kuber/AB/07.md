...
1: generate a private key file `ca.key`

```bash
openssl genrsa -out ca.key 2048
```

2: create a CertificateSignRequest file `ca.csr` by `ca.key`

```bash
openssl req -new -key ca.key -subj "/CN=KUBERNETES-CA" -out ca.csr
```

3: sign the created `ca.csr` and create `ca.crt` files
...
1: generate a private key file `admin.key`

```bash
openssl genrsa -out admin.key 2048
```

2: create a CertificateSignRequest file `admin.csr` by `admin.key`

```bash
openssl req -new -key admin.key -subj "/CN=kube-admin \O=system:masters" -out admin.csr
```

3: sign the created `admin.csr` and create `admin.crt` file
...
2: generate a private key file `apiserver.key`

```bash
openssl genrsa -out apiserver.key 2048
```

3: create a ***C***ertificate***S***igning***R***equest `apiserver.csr` by `apiserver.key`
...
4: sign the created `apiserver.csr` and create `apiserver.crt` file
...
1: User Create ***C***ertificate***S***igning***R***equest Object
